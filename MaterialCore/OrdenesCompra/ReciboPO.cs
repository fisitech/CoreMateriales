...
namespace MaterialCore
{
    public partial class ReciboPO : Form
    {
      ...
      //  Twain.Principal objTW = new Twain.Principal();
        ...
        private int _Materiales_Paletas_Id;
        ...
        //las validaciones entorpesen el sistema, se puede agregar lineras con misma po linea, guia.... pero diferente factura provedor
            ...
            //}
                ...
                if(!ValidaPartida( Convert.ToInt64(txtNumeroPo.Text)   ,70, Convert.ToInt32(txtRenglonID.Text), txtNumeroParte.Text,txtFacturaProveedor.Text))
                {
                    ...
                    //int IdCliente = 70;
                    //int Renglon = Convert.ToInt32(txtRenglonID.Text);
                    //string NoParte = txtNumeroParte.Text.Trim();
                    //string FacturaProveedor = txtFacturaProveedor.Text;
                    string NumeroPaleta, NombrePaleta, Consecutivo, FechaEmbarque;

                    if (_Materiales_Paletas_Id != 0)
                    {
                        Clases.Conexion conn = new MaterialCore.Clases.Conexion();
                        SqlDataReader datos;

                        string sql = string.Format("Select Distinct(mp.NumeroPaleta), mp.NombrePaleta, mp.ConsecutivoEmbarque, mp.FechaEmbarque    " +
                                                                   "From MaterialesPaltetas mp   " +
                                                                    "Join MaterialesEnviar me   " +
                                                                    "On me.MaterialesPaletasId =  mp.Id  " +
                                                                   "WHERE (me.MaterialesPaletasId = {0})     ", _Materiales_Paletas_Id);
                       datos = conn.getLector(sql);
                         if (datos.Read())
                         {
                             NumeroPaleta = datos["NumeroPaleta"].ToString();
                             NombrePaleta = datos["NombrePaleta"].ToString();
                             Consecutivo = datos["ConsecutivoEmbarque"].ToString();
                             FechaEmbarque = datos["FechaEmbarque"].ToString();
                         //}
                         if (DialogResult.Yes ==
                             MessageBox.Show("Esta partida ha sido capturada previamente, Numero Paleta: " + NumeroPaleta.ToString() + ", Nombre Paleta: " + NombrePaleta.ToString() + ", Consecutivo: " + Consecutivo.ToString() + ", Fecha Embarque: " + FechaEmbarque.ToString() + " ¿Desea continuar con esta captura?"
                    ...
                    {
                        if (DialogResult.Yes ==
                                              MessageBox.Show("Esta partida ha sido capturada previamente, en la fecha: " + _Fecha + " ¿Desea continuar con esta captura?"
                                              , "Aviso", MessageBoxButtons.YesNo,
                                               MessageBoxIcon.Question, MessageBoxDefaultButton.Button2))
                        {
...
decimal Precio
    ...
    /*        for (int i = 0; i < dgvPos.RowCount; i++)
        ...
        bool ValidaPartida(System.Int64 PO, int IdCliente, int Renglon, string NoParte, string FacturaProveedor)
        {
            ...
            string sql = string.Format("SELECT Planta, isnull(MaterialesPaletasId,0) as MaterialesPaletasId, Fecha    " + //añadi campo fecha
            ...
            if (datos.Read())
            {
                planta = datos["Planta"].ToString();
                paleta = Convert.ToInt32(datos["MaterialesPaletasId"]);
                _Materiales_Paletas_Id = paleta;
                _Fecha = datos["Fecha"].ToString();
                resultado = false;
            }
                  return resultado;
        }
